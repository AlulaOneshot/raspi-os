use shgl::{Color, ShGLContext, Texture, Vec3};

fn main() {
    let mut ctx = ShGLContext::new();
    match ctx.init() {
        Ok(()) => {
            let texture = Texture::load_from_file("../texture.jpg").unwrap();

            while !ctx.should_close() {
                ctx.handle_events();
                ctx.begin_drawing_upper();
                ctx.clear_screen(Color::from_hex(0xFF0000FF));
                ctx.draw_triangle_textured([Vec3::new(-1.0, -1.0, 0.0), Vec3::new(1.0, -1.0, 0.0), Vec3::new(-1.0, 1.0, 0.0)], [Vec2::new(-1.0, -1.0), Vec2::new(1.0, -1.0), Vec2::new(-1.0, 1.0)], texture);
